...
const [data, stat] = await (async function () {
  ...
  if (!path) {
    console.log(
      "Could not find `package.nw`. Please check `--package` argument"
    );
    process.exit(1);
  }
  ...
  context.body = await (async function () {
    if (path === "index.html") {
      ...
      body = body.replace(
        header,
        `<script>
if (localStorage.backendDomain && localStorage.backendDomain !== ${JSON.stringify(
          info.backend
        )}) {
        ...
        const version = await (async function () {
          try {
            const response = await fetch(
              `${argv.internal_backend ?? info.backend}/api/version`
            );
